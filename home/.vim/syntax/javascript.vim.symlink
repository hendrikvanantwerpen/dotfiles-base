...
syntax region  javaScriptLineComment    start=+\/\/+ end=+$+ keepend contains=javaScriptCommentTodo,@Spell ""fold
...
if exists("b:javascript_fold")
    ...
    if &l:filetype=='javascript' && !&diff
      ...
      function! MyFoldText()
        let tabspace = repeat( ' ', getwinvar( 0, '&shiftwidth' ) )
        let line = getline(v:foldstart)
        let sub = line " substitute(line, '/\*\|\*/\|{{{\d\=', '', 'g')
        let sub = substitute(sub, '\t', tabspace, 'g')

        let line = getline(v:foldend)
        let subend = substitute(line, '.*\(\*/\|}}}\d\=\)', '\1', 'g')
        let subend = substitute(subend, '^\s*', '', 'g')
        let sub = sub . " ... " . subend

        " would be nice to add end parens and characters after that, e.g. comma's now missing
        ...
        let filling = repeat (' ', winwidth(0) - strlen( sub ) - strlen( info ) - num_w - fold_w - 1  )
        return sub . filling . info
      endfunction
